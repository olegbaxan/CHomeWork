...
while(1){
	...
	if (int(pass[i])==8){
		...
		if (strcmp(S[User_Iter].pass,pass)==0){cout<<" Password OK"<<endl;userType=S[User_Iter].type;statAuth();break;}
		else {
...
void findUser(int x){
	...
	switch(x){
		...
		case 3:{cout<<"Enter type to search"<<endl;cin>>userType;typex=1;}  break;
	...
	for(int i=0;i<Nuser;i++)
	{	
		if(((numex==1)&&(strcmp(CONT[i].userID,username)==0))||((mailx==1)&&(strcmp(CONT[i].emailUser,mail)==0))||((typex==1)&&(CONT[i].type==userType))||((statx==1)&&(CONT[i].status==status))||((idx==1)&&(CONT[i].id==userid))){
...
int insertNumber(char* mess, int i)
{
	cout<<mess<<" ["<<i+1<<"]: ";
    	...
    	do{ 
        	...
        	if (int(pass[p])==8){
				if(p>0){
					cout<<"\b \b";//rescrierea unei numar inserat gresit
			}}
			else if((int(pass[p])>=48)&&(int(pass[p])<=57)){ 
            	cout<<pass[p]; 
        	}
        	p++; 
    	}while(int(pass[p-1])!=13); 
    	pass[p-1]='\0'; 
    	return idnp=atoi(pass);
}
int insertStud(stud S[], int n)
{
	...
	do 
	{
		...
		studNumber=insertNumber("Enter IDNP for student",i);
		if (checkStudIDNP(studNumber)){
		S[i].IDNP=studNumber;
		cout<<"\nEnter nume student ["<<i+1<<"]: ";
		cin>>S[i].nume;
		cout<<"Enter prenume student ["<<i+1<<"]: ";
		cin>>S[i].prenume;
		S[i].varsta=insertNumber("Enter anul nasterii for student",i);
...
{	cout<<"Please enter Students name to change"<<endl;
	cin>>studname;
	//cout<<"Please enter Students anul to change"<<endl;//Back-up for changing name if name==NULL
	//cin>>studInt;//Back-up for changing name if name==NULL
	for (int i=0;i<Nstud;i++){
		if (strcmp(STUD[i].nume,studname)==0){//cout<<"x="<<x;system("pause");
		//if(STUD[i].varsta==studInt){//Back-up for changing name if name==NULL
			if (STUD[i].status==0&&userType==0){
				cout<<"Student inactive "<<userType ;//system("pause");
			}
			else{
				switch(x){
					...
					case 3:{cout<<"You want to change student anul nasterii? ESC - exit, anykey continue ..."<<endl;
						...
						STUD[i].varsta=insertNumber("Enter anul nasterii for student",i);statEdit();
						}}break;
					case 4:{cout<<"You want to change student IDNP? ESC - exit, anykey continue ..."<<endl;
						...
						studNumber=insertNumber("Insert IDNP for student ", i);
						if (checkStudIDNP(studNumber)){
